...
export default function GasPage() {
  ...
  if (!gas) return <Spinner label="Loading gas prices…" />;

  const costUsd = (gwei: number) => (gwei * 1e-9 * TX_GAS * gas.ethUsd);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Ethereum mainnet gas prices · ETH @ ${gas.ethUsd.toLocaleString()} · updated {fmtDate(gas.updatedAt, true)}</p>
        </div>
