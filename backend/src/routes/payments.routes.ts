...
import { createCheckout, handleWebhook, myOrders, settleOrder, verifyOnchainPayment } from '../services/payments.service';
...
paymentsRouter.post('/checkout', authRequired, asyncHandler(async (req, res) => {
  ...
  res.status(201).json({
    ...
    note: order.invoiceUrl
      ? 'Hosted invoice created — open the invoice URL to pay.'
      : method === 'metamask'
        ? 'Send the exact amount, then confirm in your wallet.'
        : 'Invoice created — pay to the address above.',
  });
}));
...
/** POST /api/payments/:id/confirm-metamask — buyer signed payment message with wallet (signature/demo mode) */
...
/** POST /api/payments/:id/verify-tx — verify a REAL on-chain ETH transfer, then settle (transaction mode) */
paymentsRouter.post('/:id/verify-tx', authRequired, asyncHandler(async (req, res) => {
  const { txHash } = req.body || {};
  if (!txHash || !/^0x[a-fA-F0-9]{64}$/.test(txHash)) return res.status(400).json({ error: 'A valid txHash (0x… 64 hex chars) is required' });
  const order = await verifyOnchainPayment(req.params.id, txHash);
  res.json({ order });
}));
